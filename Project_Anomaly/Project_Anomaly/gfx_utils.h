...
int filledTri(SDL_Renderer* renderer, Triangle t);
int filledTri(SDL_Renderer* renderer, vec2 p0, vec2 p1, vec2 p2);
int filledTri(SDL_Renderer* renderer, int x0, int y0, int x1, int y1, int x2, int y2);

std::vector<int> interpolateInts(int y0, int x0, int y1, int x1);

int filledTri(SDL_Renderer* renderer, Triangle t)
{
	return filledTri(renderer, t.p0, t.p1, t.p2);
}

int filledTri(SDL_Renderer* renderer, vec2 p0, vec2 p1, vec2 p2) 
{
	return filledTri(renderer,
		p0.x, p0.y,
		p1.x, p1.y,
		p2.x, p2.y);
}
